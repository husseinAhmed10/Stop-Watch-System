...
void Timer1_init_CTC() {
	...
	OCR1A = 977;	//compare value of the timer using prescalar 1024
...
ISR(INT0_vect) {
	...
	//assumption the timer was stopped then reset
	TCCR1B |= (1 << CS12) | (1 << CS10);
	//Additional
	PORTC |= (1 << PC6);	//LED GREEN COUNTING ON
	PORTC &= ~(1 << PC7);	//LED RED STOP OFF
...
ISR(INT1_vect) {
	...
	//additional
	PORTC &= ~(1 << PC6);	//LED GREEN COUNTING OFF
	PORTC |= (1 << PC7);	//LED RED STOP ON
...
int main(void) {
	...
	//Additional feature led on when counting and other one when stop
	DDRC |= (1 << PC7) | (1 << PC6);
	PORTC |= (1 << PC6);	//LED GREEN COUNTING ON
	PORTC &= ~(1 << PC7);	//LED RED STOP OFF
